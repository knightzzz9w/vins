...
ros::Publisher pub_keyframe_point_incre;
...
void registerPub(ros::NodeHandle &n)
{
    ...
    pub_keyframe_point_incre = n.advertise<sensor_msgs::PointCloud>("keyframe_point_incre", 1000);
...
void pubKeyframe(const Estimator &estimator)
{
    ...
    if (estimator.solver_flag == Estimator::SolverFlag::NON_LINEAR && estimator.marginalization_flag == 0)   //次新帧是关键帧
    {
        int i = WINDOW_SIZE - 2;
        //Vector3d P = estimator.Ps[i] + estimator.Rs[i] * estimator.tic[0];
        ...
        for (auto &it_per_id : estimator.f_manager.feature)  //所有的feature 列表
        {
        ...
        }  //这里publish incre pointscloud


        sensor_msgs::PointCloud point_cloud_incre;
        point_cloud_incre.header = estimator.Headers[WINDOW_SIZE - 2];
        ROS_DEBUG("feature incre size is %d", estimator.f_manager.feature_incre.size());
        for (auto &it_per_id_map : estimator.f_manager.feature_incre)  //所有的feature 列表
        {
            auto &it_per_id = it_per_id_map.second;
            int frame_size = it_per_id.feature_per_frame.size();
            if(it_per_id.start_frame < WINDOW_SIZE - 2 && it_per_id.start_frame + frame_size - 1 >= WINDOW_SIZE - 2 && it_per_id.solve_flag == 1)//这边solve flag代表三角化成功
            {
                ...
                p.y = w_pts_i(1);
                p.z = w_pts_i(2);
                point_cloud_incre.points.push_back(p);
                ...
                p_2d.values.push_back(it_per_id.feature_per_frame[imu_j].uv.y());
                p_2d.values.push_back(it_per_id.feature_id);
                point_cloud_incre.channels.push_back(p_2d);
            }

        }  //这里publish incre pointscloud
        ROS_DEBUG("points incre size is %d", point_cloud_incre.points.size());
        pub_keyframe_point.publish(point_cloud);
        pub_keyframe_point_incre.publish(point_cloud_incre);
    }
}
